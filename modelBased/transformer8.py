...
import copy
"""
终极思路, 重写postion encoding
要是不行 我就心态崩了
nibushi youliangge keyide moxingma
...
class ExtractionModule(nn.Module):
    def __init__(self, action_dim, embed_dim, num_heads):
        ...
        self.dir_embedding = nn.Linear(1, embed_dim)
        self.attention = nn.MultiheadAttention(embed_dim, num_heads=num_heads, batch_first=True)
        self.position_embedding = nn.Embedding(72, embed_dim)
        self.query_fc = nn.Linear(3 * embed_dim, embed_dim)
        self.value_fc = nn.Linear(2 * embed_dim, embed_dim)
    ...
    def forward(self, state, action):
        ...
        agent_position1D = agent_position[:, 0] * 12 + agent_position[:, 1]
        agent_position_embed = self.position_embedding(agent_position1D).unsqueeze(1)
