...
class ActorCritic(nn.Module):
    ...
    def act(self, state, visit_counts=None):
        ...
        if self.has_continuous_action_space:
            ...
            action_probs = self.adjust_action_probs(action_probs, state, visit_counts)
    ...
    def adjust_action_probs(self, action_probs, state, visit_counts, VISIT_THRESHOLD=10, EXPLORATION_BONUS=0.1):
        state_key = tuple(state.cpu().numpy())
        adjusted_probs = action_probs.clone()

        for action in range(len(action_probs)):
            key = (state_key, action)
            visit_count = visit_counts.get(key, 0)
            if visit_count < VISIT_THRESHOLD:
                adjusted_probs[action] += EXPLORATION_BONUS
        adjusted_probs /= adjusted_probs.sum()
        return adjusted_probs

    def evaluate(self, state, action, visit_counts):
...
class PPO:
    ...
    def __init__(self, state_dim, action_dim, lr_actor, lr_critic, gamma, K_epochs, eps_clip,
                 has_continuous_action_space, R_max, VISIT_THRESHOLD, action_std_init=0.6):
    ...
    def get_modified_reward(self, state, action, reward):
        key = self.get_state_action_key(state, action)
        visit_count = self.visit_counts.get(key, 0)
        if visit_count < self.VISIT_THRESHOLD:
            return self.RMAX_VALUE  # positive reward
        else:
