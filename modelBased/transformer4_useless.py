...
class ExtractionModule(nn.Module):
    def __init__(self, action_dim, embed_dim, num_heads):
        ...
        self.action_embedding = nn.Linear(action_dim, (embed_dim-2) // 2)
    ...
    def extract_topk_regions(self, attention_weights, value, topk=20):
        """
        extract topk regions from the attention weights
        """

        _, topk_indices = torch.topk(attention_weights[:,0,:], topk, dim=1)  # (batch_size, topk)
        mask = torch.zeros_like(attention_weights[:,0,:])
        mask.scatter_(-1, topk_indices, 1.0)  
        attention_weights_mask = attention_weights[:,0,:] * mask
        attention_weights_mask = attention_weights_mask / attention_weights_mask.sum(dim=-1, keepdim=True)  
        attention_weights_mask = attention_weights_mask / attention_weights_mask.max()
    ...
    def forward(self, state, action):
        ...
        dir_emdedding = self.dir_embedding(dir.unsqueeze(1)).unsqueeze(1)  # (128, 1, 62)
        position = torch.arange(state_embed.size(1), device=state.device).unsqueeze(1)  # (seq_len, 1)
        position_encoding = self.generate_positional_encoding(position, state_embed.size(-1)) 

        position_encoding = position_encoding.unsqueeze(0).expand(state.size(0), -1, -1)  # (128, 72, 2)
        state_embed = torch.cat([state_embed, position_encoding], dim=-1)  # (128, 72, 64)
        ...
        action_embed = torch.cat([action_embed, position_embeddin_temp, dir_emdedding], dim=-1) # (128, 1, 64)
        # action_embed = torch.cat([action_embed, dir_emdedding], dim=-1) # (128, 1, 64)
        _, attention_weights = self.attention(query=action_embed, key=state_embed, value=state_embed)
        attention_output = self.extract_topk_regions(attention_weights, state_embed, topk=20)
...
class PredictionModule(nn.Module):
    def __init__(self, embed_dim, state_dim, hidden_dim=128):
        ...
        self.fc3 = nn.Linear(hidden_dim, 27)
...
class IntegratedPredictionModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        obs_next_temp = obs_next.view(obs_next.shape[0], 3, 3, 3)  # convert to (batch_size, width, height, channels)
    ...
    def visualization(self, obs, act, attentionWeight, obs_next, obs_pred):
        ...
        # for predict change
        obs_next_temp = obs_next.view(obs_next.shape[0], 3, 3, 3)   # convert to (batch_size, width, height, channels)
        if round(obs_next_temp[-1, 2, :, :].detach().cpu().numpy().max() * 3) not in self.direction_map.keys():
            next_direction = "Unknown"
        else:
        ...
        dir = round(obs_pred[-1, :].reshape(3, 3, 3)[2, :, :].detach().cpu().numpy().max() * 3)
        ...
        obs_pred = np.round(obs_pred[-1, :].reshape(3, 3, 3)[0, :, :].detach().cpu().numpy() * 10)  #  convert tensor to numpy
