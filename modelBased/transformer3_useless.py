...
class ExtractionModule(nn.Module):
    def __init__(self, action_dim, embed_dim, num_heads):
        ...
        self.position_embed = nn.Embedding(11, embed_dim//3)   # valid_values_obj: [1, 2, 4, 5, 8, 10]  0-10  -> 11
        self.color_embed = nn.Embedding(6, embed_dim//3)       # valid_values_obj: [1, 2, 3, 4, 5, 6]  0-5  -> 6
        self.direction_embed = nn.Embedding(4, embed_dim//3)   # valid_values_obj: [1, 2, 3, 4]  0-3  -> 4
        self.action_embed = nn.Embedding(3, embed_dim//4)      # valid_values_obj: [0, 1, 2]  0-2  -> 3  
        self.attention = nn.MultiheadAttention(embed_dim, num_heads=num_heads, batch_first=True)
        self.state_linear = nn.Linear(embed_dim, embed_dim//4)

    def forward(self, state, action):
        position_emb = self.position_embed(state[:, 0, :, :].flatten(1).long())
        color_emb = self.color_embed(state[:, 1, :, :].flatten(1).long())
        direction_emb = self.direction_embed(state[:, 2, :, :].flatten(1).long())
        state_emb = torch.cat((position_emb, color_emb, direction_emb), dim=-1)
        action_emb = self.action_embed(action.unsqueeze(1).long())

        # state_emb = self.state_linear(state_emb)
        attention_output, attention_weights = self.attention(query=action_emb, key=state_emb, value=state_emb)
        ...
        # dir_emdedding = dir.unsqueeze(-1).unsqueeze_(-1)
...
class IntegratedPredictionModel(pl.LightningModule):
    ...
    def visualization(self, obs, act, attentionWeight, obs_next, obs_pred):
        ## preporcessing data
        state_image = obs[-1, 0, :, :].detach().cpu().numpy()   # convert tensor to numpy
        direction = self.direction_map[round(obs[-1, 2, :, :].detach().cpu().numpy().max())]
        action = self.action_map[round(act[-1].item())]
        ...
        next_direction = self.direction_map[round(obs_next_temp[-1, 2, :, :].detach().cpu().numpy().max())]
        obs_next = obs_next_temp[-1, 0, :, :].detach().cpu().numpy()   # convert tensor to numpy
        dir = round(obs_pred[-1, :].reshape(3, 6, 12)[2, :, :].detach().cpu().numpy().max())
        if dir not in self.direction_map.keys():
            pre_direction = "Unknown"
        else:
            pre_direction = self.direction_map[dir]
        obs_pred = np.round(obs_pred[-1, :].reshape(3, 6, 12)[0, :, :].detach().cpu().numpy())  #  convert tensor to numpy
