...
import modelBased.AttentionWM_support as AttentionWM_support
...
class IntegratedModel(nn.Module):
    def __init__(self, feature_extration_mode, batch_size, grid_shape, mask_size, delta_shape, embed_dim, num_heads, freeze_weight=False, weight_path=''):
        super().__init__()
        self.extraction_module = AttentionWM_support.ExtractionModule(feature_extration_mode, batch_size, grid_shape, mask_size, embed_dim, num_heads)
        self.prediction_module = AttentionWM_support.PredictionModule(embed_dim, delta_shape)
        ## Freaze the extraction module & Local test
        if freeze_weight:
            utils.load_module_weight(self.extraction_module, weight_path, 'extraction')
            utils.load_module_weight(self.prediction_module, weight_path, 'prediction')
