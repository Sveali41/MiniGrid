...
class SimpleNN(nn.Module):
    def __init__(self, obs_shape, next_obs_shape, action_shape, hidden_size):
        ...
        self.state_head = nn.Sequential(nn.Linear(hidden_size, next_obs_shape), nn.ReLU())
    ...
    def forward(self, input_obs, input_action):
        ...
        reward_out = self.reward_head(out)
...
def test_policy(policy, env, num_episodes=100):
    ...
    for _ in range(num_episodes):
        ...
        while not done:
            ...
            action = action_prob.max(1)[1].view(1, 1).item()
...
if __name__ == "__main__":
    # test the model
    loaded_model = SimpleNN(147, 147, 1, 50)
    path = Paths()
    env_0 = ImgObsWrapper(
    ...
    image_shape = np.prod(env_0.observation_space.shape)
