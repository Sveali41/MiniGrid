...
class ExtractionModule(nn.Module):
    def __init__(self, mode, batch_size, grid_shape, mask_size, embed_dim, num_heads, ff_dim=128, dropout=0):
        super().__init__()
        ff_dim = 2 * embed_dim
        self.mode = mode
        ...
        if mode == 'fc':
            self.fc_state = nn.Sequential(
                        nn.Linear(grid_shape[0] * mask_size *  mask_size, 4 * embed_dim),
...
class PredictionModule(nn.Module):
    def __init__(self, embed_dim, delta_shape, hidden_dim=128):
        ...
        self.fc4 = nn.Linear(hidden_dim // 2, delta_shape[0] * delta_shape[1] * delta_shape[2])
