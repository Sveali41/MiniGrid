...
def run(cfg: DictConfig):
    ...
    # generate final task
    final_task_set = support.generate_final_task(20, 20, 2)
    # train the policy in the final task set
    # if we use regret for the performance evaluation 
    if cfg.PPO.performance_eval_method == 'regret':
        cfg.PPO.compute_regret = True
    ...
    for step in range(step_len):
        ...
        print("++++++++++++++++++++++++++++++++++++ 2. collecting data for world model training... ++++++++++++++++++++++++++++++++++++++++++++++")
        ...
        cur_old_params, cur_fisher = AttentionWM_training.train_api(cfg, old_params, fisher, env_layout)
        ...
        print("++++++++++++++++++++++++++++++++++++ 3. training the policy in the final-task set... ++++++++++++++++++++++++++++++++++++++++++++++")
        if cfg.PPO.performance_eval_method == 'regret':
            regret = []
        ...
        #                      '/home/siyao/project/rlPractice/MiniGrid/modelBased/models/PPO_gen_final_task_4.ckpt',]
            ...
            print("++++++++++++++++++++++++++++++++++++ 4. evaluate the performance... ++++++++++++++++++++++++++++++++++++++++++++++")
        ...
        if cfg.PPO.performance_eval_method == 'average_reward':
            rewards = []
            for i in range(len(final_task_set)):
                cfg.PPO.env_path = final_task_set[i]
                cfg.PPO.wandb_run_name = f"env_{step}_final_task_{i}_policy_{datetime.now().time()}"
                PPO_world_training.run_training_wm(cfg)
                reward = PPO_world_training.run_policy_evaluation(cfg)
                rewards.append(reward)
            average_reward = sum(rewards) / len(rewards)

            print("++++++++++++++++++++++++++++++++++++ 4. evaluate the performance... ++++++++++++++++++++++++++++++++++++++++++++++")

            if average_reward > cfg.PPO.reward_threshold:
                print('************************************************')
                print(f"Step {step}: Success! The average reward is {average_reward}")
                print('************************************************')
                break
            else:
                print('*************************************************')
                print(f'Step {step}: Fail! The average reward is {average_reward}')
                print('*************************************************')
